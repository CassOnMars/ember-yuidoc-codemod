...
function run() {
  ...
  return new Promise(resolve => {
    let ps = cp.spawn('node', [
      path.join(originalCwd, 'bin/ember-yuidoc-codemod')
    ], {
      cwd: tmpPath
    });
...
describe('bin acceptance', function() {
  ...
  describe('with valid package.json', function() {
    ...
    describe('with valid file', function() {
      let tmpFile;
      const inputFile = path.join(originalCwd, '__testfixtures__/doctest.input.js');
      const outputFile = path.join(originalCwd, '__testfixtures__/doctest.output.js');


      beforeEach(function() {
        fs.ensureDirSync(path.join(tmpPath, 'app'));

        tmpFile = path.join(tmpPath, 'app/doctest.js');
