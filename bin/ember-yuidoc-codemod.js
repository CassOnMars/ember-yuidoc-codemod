...
try {
  ...
  // Generate YUIDOC_REPORT.md when jscodeshift is done running.
...
function buildReport() {
  ...
  glob("ember-yuidoc-codemod.tmp.*", (err, logs) => {
    ...
    // markdown and write it to YUIDOC_REPORT.md.
    if (report.length) {
      ...
      let file = "## YUIDOC Report\n" + report.join("\n");

      // normalize line endings, so we don't end up with mixed
      file = file.replace(/\r?\n/g, require('os').EOL);

      fs.writeFileSync("YUIDOC_REPORT.md", file);
      console.log(chalk.yellow("\nDone! Some files could not be upgraded automatically. See " + chalk.blue("YUIDOC_REPORT.md") + "."));
    } else {
